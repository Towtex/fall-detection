...
def test_by_features(output_path: str, dataset_path: str, subject: int, camera: str, trial: int, feature_str: str, class_limit: int, action: int):
    model_str = os.path.join(output_path, f'Subject_{subject}', f'train_data_{class_limit}_classes_cam_{camera}_test_subject{subject}_LOOCV_{feature_str}')
    ...
    out_subStr = f'Subject_{subject}'
    out_camStr = f'Camera_{camera}'
    out_trialStr = f'Trial_{trial}'
    out_actStr = f'Activity_{action}'
    
    subStr = f'Subject{subject}'
    camStr = f'Camera{camera}'     
    seq_length = 18
    ...
    if camera == '1_2':
        ...
        output_fld_2 = os.path.join(output_path, out_subStr, 'Camera_2', out_trialStr, out_actStr, f'{camStr}_{feature_str}_Test_LOOCV_{subStr}_{class_limit}_Classes_Results.mp4')
        ...
        fg_fld_1 = os.path.join(output_path, out_subStr, out_camStr, out_trialStr,out_actStr, 'SHI_FD')
        
        features_fld = os.path.join(output_path, out_subStr, out_camStr, f'CNN_features_sequences_{feature_str}')
        output_fld_1 = os.path.join(output_path, out_subStr, out_camStr, out_trialStr, out_actStr, f'{camStr}_{feature_str}_Test_LOOCV_{subStr}_{class_limit}_Classes_Results.mp4')
...
#     test_feature='DOF' # can change value to 'SHI_FD_FD', 'DOF_SHI_FD_FD', 'DOF_SHI_FD_Yolov8', 'DOF' ''SHI_FD_Yolov8'
