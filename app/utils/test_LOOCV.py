...
def load_model_and_data(main_folder, class_limit, subject, camera, feature):
    model_str = os.path.join(
        main_folder,
        f'Subject_{subject}',
        f'train_data_{class_limit}_classes_cam_{camera}_test_subject{subject}_LOOCV_{feature}'
    )
...
def generate_confusion_matrix(model, x_test, y_test, class_limit, display_labels, file_writer):
    ...
    conf_mat = tf.math.confusion_matrix(labels=actual_classes, predictions=predicted_classes, num_classes=int(class_limit))
...
def test_loocv(feature: str, class_limit: str, subject: str, camera: str):
    # Main test function
    ...
    model, x_test, y_test, model_str = load_model_and_data(main_folder, class_limit, subject, camera, feature)
    log_file = f'{model_str}_performance_eval.txt'
    file_writer = open(log_file, 'w')
    print("Starting evaluation...")
    display_labels = get_display_labels(int(class_limit))
    ...
    conf_mat, cm_display, report, actual_classes, predicted_classes = generate_confusion_matrix(
        model, x_test, y_test, int(class_limit), display_labels, file_writer)
    ...
    save_results(results, model_str, cm_display, int(class_limit))
    file_writer.close()
