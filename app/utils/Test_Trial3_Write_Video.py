...
#     camera='2' # for training both camera 1 and 2 data, change value to '1_2' '4096' . otherwise change to '1' or '2' '2048' for training each camera data separately
#     model_str = f'{dataset_path}/train_data_{class_limit}_classes_cam_{camera}_test_trial3_{test_feature}'
    
#     #log_file= f'{model_str}_performance_eval.txt'
#     #file_writer = open(log_file, 'w')
#     #print(type(file_writer))
   
#     model_fld = f'{model_str}/weight_models/'
#     all_model_files = glob.glob(os.path.join(model_fld,'*.hdf5'))
#     all_model_files.sort(key=os.path.getctime, reverse=True)
